...
class GraphicalUI:
    ...
    def setup_frames(self):
        ...
        self.rootimageLabel = tk.Label(self.root)
        self.rootimageLabel.pack()

        self.background = ImageTk.PhotoImage(file = "images/background/background.png")
        self.rootimageLabel.configure(image = self.background)
        self.rootimageLabel.image = self.background
    ...
    def create_stock_buttons(self, stocks):
        for i, j in zip(range(1, len(stocks)+1), stocks):
            button = tk.Button(self.stock_frame, bd = 1, relief = 'ridge', text = "%s"%(j), command = partial(self.plot, '15 min', j))
    ...
    def create_time_buttons(self, stock, time_stamps = ['15 min', '1 H', '1 D', '1 W']):
        for i, j in zip(range(0, len(time_stamps)), time_stamps):
            button = tk.Button(self.time_frame, bd = 1, relief = 'ridge',text = "%s"%(j), command = partial(self.plot, j, stock))
    ...
    def graph_visualisation(self, x_axis, y_axis, custom, label, duration):
        ...
        fig.add_trace(go.Scatter(x = x_axis, y = y_axis, customdata = custom, hovertemplate = '<br>'
                        .join(['Open : $%{y:.2f}', 'Date : %{x}', '%{customdata[0]}', '%{customdata[1]}', '%{customdata[2]}']),))
